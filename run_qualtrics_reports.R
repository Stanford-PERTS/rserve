...
known_qualtrics_programs <- c('ep')
if (!script_path %in% known_qualtrics_programs) {
  stop(paste(script_path, "is not a recognized qualtrics program script."))
}
...
report_data_list <- script$main(
  ...
  qualtrics_service = qualtrics$create_service(request$payload$qualtrics_credentials$api_key),
  saturn_service = NULL,
  sheets_service = NULL,
